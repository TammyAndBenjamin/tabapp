...
class ContactForm(Form):
    ...
    lang = QuerySelectField(label=lazy_gettext('Language'), get_label=lambda obj: obj.label, query_factory=lambda: Locale.query.filter(Locale.type=='lang'), allow_blank=True)
    tz = QuerySelectField(label=lazy_gettext('Timezone'), get_label=lambda obj: obj.label, query_factory=lambda: Locale.query.filter(Locale.type=='tz'), allow_blank=True)
    roles = MultiCheckboxField(label=lazy_gettext('Roles'), get_label=lambda obj: obj.name, query_factory=lambda: Role.query.filter())
